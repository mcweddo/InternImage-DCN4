...
# Updated BaseModule import from MMEngine:
from mmengine.model import BaseModule
from mmdet.models.builder import BACKBONES
from torch.nn.modules.batchnorm import _BatchNorm

from .intern_image import InternImage  # Assuming InternImage is already updated
...
class _InternImage(InternImage):
    """InternImage variant for cascade backbones in CBInternImage."""

    def __init__(self, cb_idx, **kwargs):
        super().__init__(**kwargs)  # Modern super() call with any extra kwargs (e.g., init_cfg)
        self.cb_idx = cb_idx
...
class CBInternImage(BaseModule):
    def __init__(self, channels=96, out_indices=None, cb_zero_init=True, cb_del_stages=1, **kwargs):
        # Pass extra keyword arguments to BaseModule (e.g., init_cfg)
        super().__init__(**kwargs)
        self.cb_zero_init = cb_zero_init
        self.cb_del_stages = cb_del_stages
        self.out_indices = out_indices
        assert len(out_indices) == 2, "out_indices must be a list/tuple of 2 elements."
