...
from mmcv.cnn.bricks.registry import (
    DROPOUT_LAYERS, FEEDFORWARD_NETWORK, TRANSFORMER_LAYER_SEQUENCE)
...
def build_MLP(input_dim, hidden_dim, output_dim, num_layers):
    # TODO: It can be implemented by adding an out_channel arg of
    ...
    # Note: The ReLU activation here is not in-place as in the default FFN configuration.
...
class DinoTransformerDecoder(DeformableDetrTransformerDecoder):
    ...
    def _init_layers(self):
        self.ref_point_head = build_MLP(self.embed_dims * 2,
                                        self.embed_dims,
                                        self.embed_dims,
                                        2)
    ...
    def gen_sineembed_for_position(self, pos_tensor):
        # pos_tensor: [n_query, bs, _]
        scale = 2 * math.pi
        # Generate a vector for computing sine/cosine positional encodings
        dim_t = torch.arange(self.embed_dims // 2, dtype=torch.float32, device=pos_tensor.device)
    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            # Prepare the reference points input according to the last dimension
            ...
            if self.with_rp_noise and self.training:
                ...
                noise = torch.rand(b, n, d, device=device) * 0.02 - 0.01
